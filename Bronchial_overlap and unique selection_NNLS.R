...
#FDR 0.5
#x <-readr::read_csv(
#  file.path(data.dir , "bronchial_all-DEGs_mild.vs.controls-NNLS-FDR0.05FC2.csv"))
#df1 <- data.frame(x)

#df1 <- select(df1, hgnc_symbol, logFC, FDR)
#df1_list <- df1 [,-2:-3]
...
x <-readr::read_csv(
  file.path(data.dir, "bronchial_all-DEGs_severe.vs.controls-NNLS-FDR0.05FC2.csv"))
table2 <- data.frame(x)
...
### Severe vs. mild COPD 
x <-readr::read_csv(
  file.path(data.dir, "bronchial_all-DEGs_severevsmild-FDR0.05FC2.csv"))
table3 <- data.frame(x)
...
dim (unique_severe)
readr::write_csv(unique_severe, file = file.path (results.dir, "bronchial_unique-DEGs_severeCOPD-NNLS-FDR0.05FC2.csv"))

unique_severe_pos <- overlap_pos
readr::write_csv(unique_severe_pos, file = file.path (results.dir, "bronchial_unique-UP-DEGs_severeCOPD-NNLS-FDR0.05FC2.csv"))

unique_severe_neg <- overlap_neg 
readr::write_csv(unique_severe_neg, file = file.path (results.dir, "bronchial_unique-NEG-DEGs_severeCOPD-NNLS-FDR0.05FC2.csv"))
...
#dim (unique_mild)
#readr::write_csv(unique_mild, file = file.path (results.dir, "mild_unique-DEGs_severeCOPD-NNLS-FDR0.05FC2.csv"))

#unique_mild_pos <- unique_mild [which(unique_mild$logFC > 1), ]
#readr::write_csv(unique_mild_pos, file = file.path (results.dir, "mild_unique-UP-DEGs_severeCOPD-NNLS-FDR0.05FC2.csv"))

#unique_mild_neg <- unique_mild [which(unique_mild$logFC < -1), ]
#readr::write_csv(unique_mild_neg, file = file.path (results.dir, "mild_unique-NEG-DEGs_severeCOPD-NNLS-FDR0.05FC2.csv"))
...
list_up <- list (df2_pos_list, df3_pos_list)
list_down <- list (df2_neg_list, df3_neg_list)
...
venn.diagram(
  ...
  main.cex = .4,
  fill = c(alpha('#C9FDE5'),alpha('#C9DEFD')),
  cex = .4,
  fontface = "bold", 
  fontfamily = "sans",
  cat.cex = 0.35,
  ...
  cat.fontfamily = "sans",
  filename =  file.path (results.dir.img, "severeCOPD_NNLS_venndiagram.png")
)
...
venn.diagram(
  ...
  cat.dist = c(0.040, 0.040),
  cat.fontfamily = "sans",
  filename =  file.path (results.dir.img, "Up-regulated_severeCOPD_NNLS_venndiagram.png")
)
...
venn.diagram(
  ...
  cat.dist = c(0.040, 0.040),
  cat.fontfamily = "sans",
  filename =  file.path (results.dir.img, "Down-regulated_severeCOPD_NNLS_venndiagram.png")
)
...
#venn.diagram(
#  x=listM, 
#  category.names = c("Severe COPD vs. control", "Mild vs. severe COPD"),
#  height = 600,
#  width = 600,
#  resolution = 300,
#  compression = "lzw",
...
#cat.dist = c(0.040, 0.040),
#  cat.fontfamily = "sans",
#  filename =  file.path (results.dir.img, "MildCOPD_NNLS_venndiagram.png")
#)
...
#  cat.fontfamily = "sans",
#  filename =  file.path (results.dir.img, "Up-regulated_mildCOPD_NNLS_venndiagram.png")
#)
...
#  cat.dist = c(0.030, 0.040, 0.040),
#  cat.fontfamily = "sans",
#  filename =  file.path (results.dir.img, "Down-regulated_mildCOPD_NNLS_venndiagram.png")
#)
