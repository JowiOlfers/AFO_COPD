...
x <-readr::read_csv(
  file.path(data.dir, "DEG.bronchial_all-DEGs_mild.vs.control-P0.01.csv"))
table1 <- data.frame(x)
...
x <-readr::read_csv(
  file.path(data.dir, "DEG.bronchial_all-DEGs_severe.vs.controls-P0.01.csv"))
table2 <- data.frame(x)
...
### Severe vs. mild COPD 
x <-readr::read_csv(
  file.path(data.dir, "DEG.bronchial_all-DEGs_severe.vs.mild-P0.01.csv"))
table3 <- data.frame(x)
...
dim (unique_severe)
readr::write_csv(unique_severe, file = file.path (results.dir, "bronchial_unique-DEGs_severeCOPD-P0.01.csv"))

#unique_severe_pos <- unique_severe [which(unique_severe$logFC > 0), ]
#readr::write_csv(unique_severe_pos, file = file.path (results.dir, "bronchial_unique-UP-DEGs_severeCOPD-P0.01.csv"))

#unique_severe_neg <- unique_severe [which(unique_severe$logFC < 0), ]
#readr::write_csv(unique_severe_neg, file = file.path (results.dir, "bronchial_unique-NEG-DEGs_severeCOPD-P0.01.csv"))
