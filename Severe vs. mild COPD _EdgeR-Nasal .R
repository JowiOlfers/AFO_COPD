...
readr::write_csv(tT1, file = file.path (results.dir, "nasal-severe.vs.mildCOPD.txt"))
...
#names(tT4)[1] <- "ENSGid"
#readr::write_csv(tT4, file = file.path (results.dir, "nasal-severe.vs.mildCOPD_P0.05.txt"))
...
dim (tT21)
readr::write_csv(tT21, file = file.path (results.dir, "nasal_up-DEGs_severe.vs.mildCOPD-FDR0.05FC2.csv"))

tT22=tT2[which(tT2$logFC <= -1), ]
dim (tT22)
readr::write_csv(tT22, file = file.path (results.dir, "nasal_down-DEGs_severe.vs.mildCOPD-FDR0.05FC2.csv"))
...
readr::write_csv(tT23a, file = file.path (results.dir, "nasal_top20.DEGs_severe.vs.mildCOPD-FDR0.05FC2.csv"))
...
readr::write_csv(tT223a, file = file.path (results.dir, "nasal_top10.DEGs_severe.vs.mildCOPD-FDR0.05FC2.csv"))


#select up and down regulated genes with FDR < 0.01 and FC > 2 or < -2
#tT31=tT3[which(tT3$logFC >= 1),] 
#readr::write_csv(tT31, file = file.path (results.dir, "nasal_up-DEGs_severe.vs.mildCOPD-FDR0.01FC2.csv"))

#tT32=tT3[which(tT3$logFC <= -1), ]
#readr::write_csv(tT31, file = file.path (results.dir, "nasal_down-DEGs_severe.vs.mildCOPD-FDR0.01FC2.csv"))
...
#readr::write_csv(tT33a, file = file.path (results.dir, "nasal_top20.DEGs_severe.vs.mildCOPD-FDR0.01FC2.csv"))
...
ph <- pheatmap(data,
              ...
              fontsize_row = 12, 
              fontsize_col = 10,
              filename = file.path (results.dir.img, "Heatmap_nasal_severe.vs.mildCOPD_FDR0.05.png") ,dpi=600)
