...
RNAseq <- read.csv(file.path(data.dir.scRNAseq_bronchial, "res4_new_means_600_0.csv"))
...
#Directory bronchial
out.dir <- file.path(results.dir, "bronchial_600")
...
saveRDS(
  hm.ciber,
  file = file.path(out.img.dir, "hm_cibersort.bronchial.rds")
)
png(
  filename = file.path(out.img.dir, "hm_cibersort.bronchial.png"),
  width = 1000,
  height = 800,
  units = "px"
)
...
res.nnls = apply(Results_nnls,2,function(x) x/sum(x))
rownames(res.nnls) <- colnames(Ref)
...
df.nnls <- res.nnls %>%
  ...
  readr::write_csv(
    file = file.path(out.dir, "nnls_result.bronchial.csv")
  )
...
saveRDS(
  hm.nnls,
  file = file.path(out.img.dir, "hm_nnls.bronchial.rds")
)
png(
  filename = file.path(out.img.dir, "hm_nnls.bronchial.png"),
  width = 1000,
  height = 800,
  units = "px"
)
...
summry_table <- df.nnls %>%
  ...
  readr::write_csv(
    file = file.path(out.dir, "result_summary.bronchial.csv")
  )


#summary table2 - no groups, 1 combined group
summry_table2 <- df.nnls %>%
  dplyr::add_row( # remove?
    df.ciber
  ) %>%
  dplyr::select(-sample) %>%
  dplyr::nest_by(method) %>%
  ...
  tidyr::unnest(summary) %>%
  readr::write_csv(
    file = file.path(out.dir, "result_summary_2.bronchial.csv")
  )
...
########################################
# Boxplot of two methods per cell type                        
########################################
...
df %>%
  readr::write_csv(
    file = file.path(out.bplot.img.dir, "boxplots.bronchial.csv")
  )

saveRDS(
  box.plt,
  file = file.path(out.bplot.img.dir, "boxplots.bronchial.rds")
)

ggsave(
  filename = file.path(out.bplot.img.dir, "boxplots.bronchial.png"),
  plot = box.plt,
  width = 20,
  height = 12.5,
  units = "cm"
...
df.nnls %>%
  readr::write_csv(
    file = file.path(out.bplot.img.dir, "boxplots.bronchial_nnls.csv")
  )

saveRDS(
  box.plt_nnls,
  file = file.path(out.bplot.img.dir, "boxplots.bronchial_nnls.rds")
)

ggsave(
  filename = file.path(out.bplot.img.dir, "boxplots.bronchial_nnls.png"),
  plot = box.plt_nnls,
  width = 20,
  height = 12.5,
  units = "cm"
...
df.ciber %>%
  readr::write_csv(
    file = file.path(out.bplot.img.dir, "boxplots.bronchial_CIBER.csv")
  )

saveRDS(
  box.plt_cib,
  file = file.path(out.bplot.img.dir, "boxplots.bronchial_CIBER.rds")
)

ggsave(
  filename = file.path(out.bplot.img.dir, "boxplots.bronchial_CIBER.png"),
  plot = box.plt_cib,
  width = 20,
  height = 12.5,
  units = "cm"
...
df.cor %>%
  readr::write_csv(
    file = file.path(out.dir, "spearman_correlation_two_methods.bronchial.csv")
  )
...
saveRDS(
  plt,
  file = file.path(out.img.dir, "scatters.bronchial.rds")
)

ggsave(
  filename = file.path(out.img.dir, "scatters.bronchial.png"),
  plot = plt
)
...
big.figure.data <- big.df2 %>%
    ...
    saveRDS(
      big.figure,
      file = file.path(out.bplot.img.dir, paste0("boxplots-cell-types-bronchial-",make.names(cell.type),"-",c.method,".rds")))
    
    
    ggsave(
      filename = file.path(out.bplot.img.dir, paste0("boxplots-cell-types-bronchial-",make.names(cell.type),"-",c.method,".png")),
      plot = big.figure,
      width = 15,
      height = 15,
      units = "cm"
    ...
    big.figure.data %>%
      readr::write_csv(
        file = file.path(out.bplot.img.dir, paste0("boxplots-cell-types-bronchial-",make.names(cell.type),"-",c.method,".csv"))
      )
...
t.test.res %>%
  readr::write_csv(file.path(out.dir, "cell-types-vs-disease-t-test.bronchial.csv"))

big.df %>%
  readr::write_csv(file.path(out.dir, "cell-types-vs-disease-values.bronchial.csv"))


### All boxplots in one figure ###
allbox_nnls <- ggarrange(celltype_bp$Ciliated_NNLS, celltype_bp$Goblet_NNLS, celltype_bp$Basal_NNLS,
          labels = c("A", "B", "C"),
          ncol = 3, nrow = 1)
ggsave(
  filename = file.path(out.img.dir, "bronchial total boxplots_NNLS.png"),
  plot = allbox_nnls,
  width = 42,
  height = 20,
  units = "cm"
...
allbox_cib <- ggarrange(celltype_bp$Ciliated_CIBERSORT, celltype_bp$Goblet_CIBERSORT,
                         labels = c("A", "B"),
                         ncol = 2, nrow = 1)
ggsave(
  filename = file.path(out.img.dir, "bronchial total boxplots_CIBER.png"),
  plot = allbox_cib,
  width = 25,
  height = 15,
  units = "cm"
...
figure_1_row_2 <- ggpubr::ggarrange(
  ...
  celltype_bp[["Ciliated_NNLS"]] + style,
  celltype_bp[["Goblet_NNLS"]] + style,
  labels = c("B", "C", "D"),
  ncol = 3,
  nrow = 1
)
...
ggsave(
  filename = file.path(out.img.dir, "Combined.bronchial_NNLS.png"),
  plot = figure_1,
  width = 20,
  height = 20,
  units = "cm"
...
figure_2_row_2 <- ggpubr::ggarrange(
    celltype_bp[["Ciliated_CIBERSORT"]] + style,
    celltype_bp[["Goblet_CIBERSORT"]] + style,
    labels = c("B", "C"),
    ncol = 2,
    nrow = 1
  )
...
ggsave(
  filename = file.path(out.img.dir, "Combined.bronchial_CIBER.png"),
  plot = figure_2,
  width = 20,
  height = 20,
  units = "cm"
