...
############################################################################################
# Use of nasal selection with CD: NNLS correction, but no cells get corrected in bronchi
############################################################################################
...
###########################DEG2 - Severe vs. mild COPD
expressionDataToUse= DEG3
samplesToUse=s3
...
readr::write_csv(tT1, file = file.path (results.dir, "DEG.bronchial-severeCOPD.vs.mild_NNLS.txt"))

tT2=tT1[which(tT1$PValue<0.01),]
readr::write_csv(tT2, file = file.path (results.dir, "DEG.bronchial-severeCOPD.vs.mild-P0.01_NNLS.txt"))

tT3=tT1[which(tT1$PValue<0.001),]
#readr::write_csv(tT3, file = file.path (results.dir, "DEG.bronchial-severeCOPD.vs.mild-P0.001_NNLS.txt"))


###select up and down regulated genes with P < 0.01 and logFC < or > 0 
tT21=tT2[which(tT2$logFC > 0),]
dim (tT21)
readr::write_csv(tT21, file = file.path (results.dir, "DEG.bronchial_up-DEGs_severe.vs.mild-P0.01_NNLS.csv"))

tT22=tT2[which(tT2$logFC < 0), ]
dim (tT22)
readr::write_csv(tT22, file = file.path (results.dir, "DEG.bronchial_down-DEGs_severe.vs.mild-P0.01_NNLS.csv"))

tT23 <- rbind (tT21, tT22)
dim (tT23)
readr::write_csv(tT23, file = file.path (results.dir, "DEG.bronchial_all-DEGs_severe.vs.mild-P0.01_NNLS.csv"))
...
readr::write_csv(tT23a, file = file.path (results.dir, "DEG.bronchial_top20.DEGs_severe.vs.mild-P0.01_NNLS.csv"))
...
readr::write_csv(tT223a, file = file.path (results.dir, "DEG.bronchial_top10.DEGs_severe.vs.mild-P0.01_NNLS.csv"))


###select up and down regulated genes with P < 0.001 and logFC < or > 0 
#tT31=tT3[which(tT3$logFC > 0),]
#dim (tT31)
#readr::write_csv(tT31, file = file.path (results.dir, "DEG.bronchial_up-DEGs_severe.vs.mild-P0.001_NNLS.csv"))

#tT32=tT3[which(tT3$logFC < 0), ]
#dim (tT32)
#readr::write_csv(tT32, file = file.path (results.dir, "DEG.bronchial_down-DEGs_severe.vs.mild-P0.001_NNLS.csv"))

#tT33 <- rbind (tT31, tT32)
#dim (tT33)
#readr::write_csv(tT33, file = file.path (results.dir, "DEG.bronchial_all-DEGs_severe.vs.mild-P0.001_NNLS.csv"))
...
#readr::write_csv(tT33a, file = file.path (results.dir, "DEG.bronchial_top20.DEGs_severe.vs.mild-P0.001_NNLS.csv"))
#
...
#readr::write_csv(tT323a, file = file.path (results.dir, "DEG.bronchial_top10.DEGs_severe.vs.mild-P0.001_NNLS.csv"))
...
p
ggsave (filename = file.path (results.dir.img, "Volcano-bronchial-severevsmild-P0.01_NNLS.png"), width=25,height=25,units="cm",dpi=600 )
ggsave (filename = file.path (results.dir.img, "gene_diff-bronchial-severevsmild-P0.01_NNLS.pdf"), width=25,height=25,units="cm")
...
write.csv(normalized_counts.P, file = file.path (results.dir, "DEG_bronchial-normalized_counts-nnls-P0.01.severevsmild.csv"))
...
write.csv(normalized_counts.Pa, file = file.path (results.dir, "DEG_bronchial-normalized_counts.top20-nnls-P0.01.severevsmild.csv"))
...
ph <- pheatmap(datax,
               ...
               fontsize_row = 12, 
               fontsize_col = 10,
               filename = file.path (results.dir.img, "Heatmap_bronchial_severe.vs.mild_P0.01_NNLS.png") ,dpi=600)
