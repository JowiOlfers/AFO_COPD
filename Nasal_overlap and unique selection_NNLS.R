...
data.dir_clinical <- file.path(".", "data")
...
x <-readr::read_csv(
  file.path(data.dir, "nasal_all-DEGs_severe.vs.controls-NNLS-FDR0.05FC2.csv"))
table2 <- data.frame(x)
...
### Severe vs. mild COPD 
x <-readr::read_csv(
  file.path(data.dir, "nasal_all-DEGs_severe.vs.mildCOPD-NNLS-FDR0.05FC2.csv"))
table3 <- data.frame(x)
...
dim (unique_severe)
readr::write_csv(unique_severe, file = file.path (results.dir, "nasal_unique-DEGs_severeCOPD-NNLS-FDR0.05FC2.csv"))

unique_severe_pos <- overlap_pos
dim (unique_severe_pos)
readr::write_csv(unique_severe_pos, file = file.path (results.dir, "nasal_unique-up-DEGs_severeCOPD-NNLS-FDR0.05FC2.csv"))

unique_severe_neg <- overlap_neg
dim (unique_severe_neg)
readr::write_csv(unique_severe_neg, file = file.path (results.dir, "nasal_unique-down-DEGs_severeCOPD-NNLS-FDR0.05FC2.csv"))
